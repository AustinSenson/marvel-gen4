...
uint16_t getAppPartition(void)
{
	...
	if (hdr->active_app == 1)
	{
		...
		RETRY(RETRY_OTA, status, verifyApp1());
		if(BMS_OK == status)    // adding retry here to prevent false switching of APP partition
		{
			return 1;
		}
		else
	...
	{
		RETRY(RETRY_OTA, status, verifyApp2());
		if(BMS_OK == status)    // adding retry here to prevent false switching of APP partition
		{
			return 2;
		}
