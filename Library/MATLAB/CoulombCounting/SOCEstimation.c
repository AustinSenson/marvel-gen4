 ...
 * C source code generated on : Sat Jul 20 16:10:28 2024
 *
...
extern bool externalLatchDetected;
...
{
  int32_T rtb_MaxAllowedImb_mV;
  int32_T rtb_MaxCurrentLimit_mA;
  int32_T rtb_MaxVoltageLimit_mV;
  int32_T rtb_MinCurrentLimit_mA;
  int32_T rtb_MinCurrentLimit_mA_tmp;
  ...
  if (SOCEstimation_B.CalcEnable > 0.0) {
    ...
    if (SOCEstimation_DW.is_active_c22_SOCEstimation == 0U) {
      ...
      if (SOCEstimation_DW.durationCounter_1 * 100 >=
        ...
        /* :  IntegratedCurrent = single(single(LoopTimer_SOC_msec)*single(Current_mA))+IntegratedCurrent; */
        if (SOCEstimation_U.Current_mA < 0) {
          rtb_MinCurrentLimit_mA_tmp = -SOCEstimation_U.Current_mA;
        } else {
          rtb_MinCurrentLimit_mA_tmp = SOCEstimation_U.Current_mA;
        }
   ...
   */
  if (SOCEstimation_U.DataPipeline.VCU.FastCharge) {
    rtb_MaxVoltageLimit_mV =
      SOCEstimation_U.CC_Inputs.FastCharge.MaxVoltageLimit_mV;
    rtb_MaxAllowedImb_mV = SOCEstimation_U.CC_Inputs.FastCharge.MaxAllowedImb_mV;
    rtb_MaxCurrentLimit_mA =
      SOCEstimation_U.CC_Inputs.FastCharge.MaxCurrentLimit_mA;
  } else {
    rtb_MaxVoltageLimit_mV =
      SOCEstimation_U.CC_Inputs.SlowCharge.MaxVoltageLimit_mV;
    rtb_MaxAllowedImb_mV = SOCEstimation_U.CC_Inputs.SlowCharge.MaxAllowedImb_mV;
    rtb_MaxCurrentLimit_mA =
  ...
  if (SOCEstimation_DW.is_active_c15_SOCEstimation == 0U) {
    ...
    switch (SOCEstimation_DW.is_c15_SOCEstimation) {
     case SOCEstimatio_IN_CoulombCounting:
      ...
      /* :  sf_internal_predicateOutput = duration(((DataPipeline.VoltageSenseBus.mV_min>=(MaxVoltageLimit_mV - MaxAllowedImb_mV)) && (abs(Current_mA)<MaxCurrentLimit_mA && abs(Current_mA)>MinCurrentLimit_mA)),msec)>DebouncingTimeout_msec && DataPipeline.VCU.isChargerConnected == 1 && CellBalancingOutput.MaxImbalance_mV<MaxAllowedImb_mV; */
      if ((rtb_MaxVoltageLimit_mV >= 0) && (rtb_MaxAllowedImb_mV <
           rtb_MaxVoltageLimit_mV - MAX_int32_T)) {
        rtb_MinCurrentLimit_mA_tmp = MAX_int32_T;
      } else if ((rtb_MaxVoltageLimit_mV < 0) && (rtb_MaxAllowedImb_mV >
                  rtb_MaxVoltageLimit_mV - MIN_int32_T)) {
        rtb_MinCurrentLimit_mA_tmp = MIN_int32_T;
      } else {
        rtb_MinCurrentLimit_mA_tmp = rtb_MaxVoltageLimit_mV -
          rtb_MaxAllowedImb_mV;
      }
      ...
      if (SOCEstimation_U.DataPipeline.VoltageSenseBus.mV_min >=
        ...
        if (rtb_MinCurrentLimit_mA_tmp < rtb_MaxCurrentLimit_mA) {
          if (SOCEstimation_U.Current_mA < 0) {
            if (SOCEstimation_U.Current_mA <= MIN_int32_T) {
              rtb_MinCurrentLimit_mA_tmp = MAX_int32_T;
            } else {
          ...
          if (rtb_MinCurrentLimit_mA_tmp > rtb_MinCurrentLimit_mA) {
          } else {
  ...
  if ((rtb_MaxVoltageLimit_mV >= 0) && (rtb_MaxAllowedImb_mV <
    ...
    rtb_MinCurrentLimit_mA_tmp = rtb_MaxVoltageLimit_mV - rtb_MaxAllowedImb_mV;
  }
  ...
  if (SOCEstimation_U.CC_Inputs.LoopTimeSelector) {
    ...
    tmp = 0.001F * SOCEstimation_B.IntegratedCurrent_h;
  }
   ...
   *  Switch: '<S3>/Switch1'
   */
  rtb_MinCurrentLimit_mA_tmp = (int32_T)floorf(0.000277777785F * tmp);
  ...
  } else if (SOCEstimation_DW.is_c27_SOCEstimation == SOCEstimation_IN_Lobby_j)
  {
    ...
    if ((SOCEstimation_B.Initial_Capacity_mAh ==
      ...
      SOCEstimation_B.TotalCapacityRemains_mAh =
        SOCEstimation_B.Initial_Capacity_mAh + rtb_MinCurrentLimit_mA_tmp;
    }
  ...
  // handling MATLAB saturation corner case
  if(SOCEstimation_B.TotalCapacityRemains_mAh < 0)
  {
    SOCEstimation_B.TotalCapacityRemains_mAh = 0;
  }
  else if(SOCEstimation_B.TotalCapacityRemains_mAh > 180000)
  {
    SOCEstimation_B.TotalCapacityRemains_mAh = 180000;
  }
   ...
   *  Product: '<S7>/Divide1'
   */
  rtb_MinCurrentLimit_mA = (int32_T)floorf((real32_T)
  ...
  if (rtb_MinCurrentLimit_mA > 10000) {
    rtb_MinCurrentLimit_mA = 10000;
  } else if (rtb_MinCurrentLimit_mA < 0) {
    rtb_MinCurrentLimit_mA = 0;
  }

  /* End of Saturate: '<S1>/Saturation1' */

  /* DataTypeConversion: '<S7>/Data Type Conversion' */
  rtb_MaxVoltageLimit_mV = (int32_T)floorf(rtb_Gain2);

  /* Saturate: '<S1>/Saturation2' */
  if (SOCEstimation_B.TotalCapacityRemains_mAh > 180000) {
    rtb_MaxCurrentLimit_mA = 180000;
  } else if (SOCEstimation_B.TotalCapacityRemains_mAh < 0) {
    rtb_MaxCurrentLimit_mA = 0;
  } else {
    rtb_MaxCurrentLimit_mA = SOCEstimation_B.TotalCapacityRemains_mAh;
  }
   ...
   *  Product: '<S3>/Divide2'
   */
  rtb_MaxAllowedImb_mV = (int32_T)floor((real_T)
    SOCEstimation_B.TotalCapacityRemains_mAh / (real_T)
    SOCEstimation_B.sf_UsableCapacity_mAh.MaxUsableCapacity_mAh * 10000.0);

  /* Saturate: '<S1>/Saturation' */
  if (rtb_MaxAllowedImb_mV > 10000) {
    rtb_MaxAllowedImb_mV = 10000;
  } else if (rtb_MaxAllowedImb_mV < 0) {
    rtb_MaxAllowedImb_mV = 0;
  }
   ...
   *  DataStoreRead: '<S1>/Data Store Read1'
   */
  u = rtb_MaxAllowedImb_mV - SOCEstimation_DW.SOC_pct;
  ...
  if (zcEvent != NO_ZCEVENT) {
    ...
    SOCEstimation_DW.SOC_pct = rtb_MaxAllowedImb_mV;
  }
  ...
  SOCEstimation_Y.CC_Outputs.Initial_Capacity_mAh =
    SOCEstimation_B.Initial_Capacity_mAh;
  SOCEstimation_Y.CC_Outputs.Total_CapacityRemains_mAh = rtb_MaxCurrentLimit_mA;
  SOCEstimation_Y.CC_Outputs.Total_Discharge_mAh = rtb_MinCurrentLimit_mA_tmp;
  SOCEstimation_Y.CC_Outputs.SOC_cpct = rtb_MaxAllowedImb_mV;
  SOCEstimation_Y.CC_Outputs.CC_State = rtb_CC_State;
  SOCEstimation_Y.CC_Outputs.MaxUsableCapacity_mAh =
    SOCEstimation_B.sf_UsableCapacity_mAh.MaxUsableCapacity_mAh;
  SOCEstimation_Y.CC_Outputs.TotalCapacityExchange_mAh = rtb_MaxVoltageLimit_mV;
  SOCEstimation_Y.CC_Outputs.SOH_cpct = rtb_MinCurrentLimit_mA;
