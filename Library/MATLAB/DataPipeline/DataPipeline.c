...
void DataPipeline_step(void)
{
   ...
   */
  DataPipeline_DW.Selector_DIMS1 = DataPipeline_U.nCells_series;
   ...
   */
  minV = DataPipeline_U.VoltageSense_mV[0];
  for (i = 1; i < DataPipeline_DW.Selector_DIMS1; i++) {
    u1 = DataPipeline_U.VoltageSense_mV[i];
    if (minV > u1) {
      minV = u1;
    }
  }
   ...
   *  SignalConversion generated from: '<S4>/Vector Concatenate'
   */
  maxV = DataPipeline_U.VoltageSense_mV[0];
  for (i = 1; i < DataPipeline_DW.Selector_DIMS1; i++) {
    u1 = DataPipeline_U.VoltageSense_mV[i];
    if (maxV < u1) {
      maxV = u1;
  ...
  memcpy(&DataPipeline_Y.DataPipelineb.VoltageSenseBus.Voltages_mV[0],
         &DataPipeline_U.VoltageSense_mV[0], 20U * sizeof(int32_T));
  ...
  for (i = 0; i < 6; i++) {
     ...
     *  Inport: '<Root>/Temperatures2_C'
     */
    u1_0 = DataPipeline_U.Temperatures2_C[i + 1];
    if (maxV_0 < u1_0) {
      maxV_0 = u1_0;
    }
     ...
     *  Inport: '<Root>/Temperatures1_C'
     */
    u1_0 = DataPipeline_U.Temperatures1_C[i + 1];
    if (maxV_1 < u1_0) {
      maxV_1 = u1_0;
    }
  }
   ...
   *  MinMax: '<S1>/Min4'
   */
  DataPipeline_MaximumFunction(DataPipeline_U.nCellTempGrps, maxV_0, maxV_1,
   ...
   *  MinMax: '<S1>/Min2'
   */
  DataPipeline_MinimumFunction(DataPipeline_U.nCellTempGrps, minV_0, minV_1,
   ...
   *  Sum: '<S1>/Subtract1'
   */
  u1_0 = (int16_T)(maxV_1 - minV_0);
  DataPipeline_Y.DataPipelineb.DeltaTemperatureGroup1_C = u1_0;
  ...
  DataPipeline_Y.DataPipelineb.TemperatureSenseBus[0].TemperatureDelta_C = u1_0;
  for (i = 0; i < 7; i++) {
  ...
  DataPipeline_Y.DataPipelineb.DCL_ChargingCurrent_A = (int32_T)floor((real_T)
  ...
  /* Gain: '<S1>/Gain7' */
  sMultiWordMul_dp(&DataPipeline_ConstP.Gain7_Gain.chunks[0], 2, &tmp_7.chunks[0U],
                2, &tmp_6.chunks[0U], 3);
