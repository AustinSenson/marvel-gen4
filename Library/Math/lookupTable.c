...
extern const uint32_t lookupTableTemps[LOOKUPTABLE_TEMPS];
extern const uint32_t socTableDischarging[LOOKUPTABLE_TEMPS][LOOKUPTABLE_NUMBERS_SIZE];
 ...
 */
uint8_t socFromLookupTable(uint8_t temperature, uint8_t voltage) 
{
 ...
 * @retval      index
 */
uint8_t findClosestIndex(uint8_t target, const uint32_t arr[], uint8_t size) 
{
  uint8_t result      = 0;
  uint8_t leftIndex   = 0;
