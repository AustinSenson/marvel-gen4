...
extern const uint16_t lookupTableTemps[LOOKUPTABLE_TEMPS];
extern const uint16_t socTableDischarging[LOOKUPTABLE_TEMPS][LOOKUPTABLE_NUMBERS_SIZE];
...
uint8_t lookup2d (Table2d *t, int32_t ix, int32_t iy, int32_t *o)
{
  ...
  /* At this point we know that the input X value is between
     ...
     data. */
  {
    const int16_t *row0 = &t->tableData[j * t->numberOfColumns];
    const int16_t *row1 = &row0[t->numberOfColumns];
    /* Difference between the two adjacent column values */
    int16_t i_delta = t->columnsData[i + 1] - t->columnsData[i];
    /* Difference between the two adjacent row values */
    int16_t j_delta = t->rowsData[j + 1] - t->rowsData[j];
    /* Low column value */
    int16_t i_low = t->columnsData[i];
    /* Low row value */
    int16_t j_low = t->rowsData[j];
    ...
    {    
      ...
      {
        int16_t o_low   = row0[i];                 /* Row value at low column # */
        int16_t o_delta = row0[i + 1] - row0[i];   /* Difference from next column */
      ...
      {
        int16_t o_low   = row1[i];                 /* Row value at low column # */
        int16_t o_delta = row1[i + 1] - row1[i];   /* Difference from next column */
    ...
    {
      *o = (uint32_t) (o0 + ((iy - j_low) * (long)(o1 - o0)) / j_delta);
      is_fault = true;
    }
  }
...
/**
 * SoC lookup and interpolation.
 *
 * @param[in]   max cell temperature      
 * @param[in]   average cell voltage
 * @retval      SoC from SoC-OCV curve
 */
uint8_t socFromLookupTable(uint8_t temperature, uint16_t voltage) 
{
  uint8_t temperatureIndex   = 0;
  uint8_t socFromLookUpTable = 0;

  // Find the closest temperature index
  temperatureIndex = findClosestIndex(temperature, lookupTableTemps, LOOKUPTABLE_TEMPS);

  // Find the closest voltage index in the corresponding temperature array
  socFromLookUpTable = findClosestIndex(voltage, socTableDischarging[temperatureIndex], LOOKUPTABLE_NUMBERS_SIZE);

  return (uint8_t)round(socFromLookUpTable);
}

/**
 * finding closest index using binary search
 *
 * @param[in]   target      
 * @param[in]   table data
 ...
 * @retval      index
 */
uint8_t findClosestIndex(uint16_t target, const uint16_t arr[], uint8_t size) 
{
  uint8_t result      = 0;
  uint8_t leftIndex   = 0;
  uint8_t middleIndex = 0;
  uint8_t rightIndex  = size - 1;
  ...
  while (leftIndex <= rightIndex) 
  {
    ...
    rightValue = arr[leftIndex];
    fraction   = (target - leftValue) / (rightValue - leftValue);
    result     =  rightIndex - fraction;
