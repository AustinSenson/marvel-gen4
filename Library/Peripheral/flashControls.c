...
bool appWriteInProgress = false;
bool sopPageErased = false;
...

{
	...
	/* 
		disable flash erase if OTA APP partition write is in progress, SoC write to flash will fail at this time. As OTA happens when behicle is on IDLE this case is solved
		if needed section parameter should be added to teh flash write function	
	*/

	if(!appWriteInProgress)
	{
	...
	{
		RETRY(RETRY_FLASH_WRITE, status, HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, startAddress, data[index]));
		startAddress += 8;
	}
	FLASH_LOCK();
...
bmsStatus_t flashErase(uint32_t startAddress)
{
	...
	switch(startAddress)
	{
		...
		case SECONDARY_APP:
			status = flashErasePage(SECONDARY_APP_PAGE_NUMBER, SECONDARY_APP_PAGES, SECONDARY_APP_BANK);
			break;
		...
		case MARVEL_SELF_DATA_ADDRESS:
			status = flashErasePage(MARVEL_SELF_DATA_PAGE_NUMBER, MARVEL_SELF_DATA_PAGE_NUMBER_OF_PAGES, MARVEL_SELF_DATA_PAGE_BANK);
			break;
		

		case SOP_DATA_ADDRESS:
			status = flashErasePage(SOP_DATA_PAGE_NUMBER, SOP_DATA_PAGE_NUMBER_OF_PAGES, SOP_DATA_PAGE_BANK);
			break;
