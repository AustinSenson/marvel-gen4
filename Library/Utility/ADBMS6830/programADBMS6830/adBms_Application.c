...
bmsStatus_t adBms6830_set_dcc_discharge(uint32_t *cellNo, uint8_t tIC, cell_asic *ic)
{
  ...
  // status = adBmsWriteData6832(tIC, &ic[0], WRPWM1, Pwm, A);
  // if(status != BMS_OK) return status;
  // status = adBmsReadData6832(tIC, &ic[0], RDPWM1, Pwm, A);
  // if(status != BMS_OK) return status;

  // status = adBmsWriteData6832(tIC, &ic[0], WRPWM2, Pwm, B);
  // if(status != BMS_OK) return status;
  // status = adBmsReadData6832(tIC, &ic[0], RDPWM2, Pwm, B);
