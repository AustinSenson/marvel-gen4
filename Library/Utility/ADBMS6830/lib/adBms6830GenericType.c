...
*******************************************************************************
*/
uint16_t adbms6830CRCErrorCount = 0;
...
*******************************************************************************
*/
bmsStatus_t adBmsReadData6832(uint8_t tIC, cell_asic *ic, uint8_t cmd_arg[2], TYPE_ADBMS6832 type, GRP group)
{
  bmsStatus_t status = BMS_ERROR;
  ...
  if(group == ALL_GRP)
  {
    ...
    else if(type == Rdasall_6832){rBuff_size = RDASALL_SIZE; regData_size = RDASALL_SIZE;}
...
*******************************************************************************
*/
bmsStatus_t spiWriteData(uint8_t tIC, uint8_t tx_cmd[2], uint8_t *data)
{
  bmsStatus_t status = BMS_ERROR;
  ...
  else
  {
    ...
    cmd[1] = tx_cmd[1];
    cmd_pec = Pec15_Calc(2, cmd);
    cmd[2] = (uint8_t)(cmd_pec >> 8);     // read abouot whats happening here tomorrow
    cmd[3] = (uint8_t)(cmd_pec);
    cmd_index = 4;
...
*******************************************************************************
*/
bmsStatus_t adBmsWriteData6832(uint8_t tIC, cell_asic *ic, uint8_t cmd_arg[2], TYPE_ADBMS6832 type, GRP group)
{	  
  bmsStatus_t status = BMS_ERROR;
    ...
    case Config_6832:	
      ...
      case A:
        ...
        for (uint8_t cic = 0; cic < tIC; cic++)
        {
          ...
          {
            write_buffer[(cic * data_len) + data] = ic[cic].configa.tx_data[data];    // loading the write_buffer pointer with necessary information commands
          }
        }
        break;
      case B:
  ...
  // adBmsWakeupIc(tIC);
  status = spiWriteData(tIC, cmd_arg, &write_buffer[0]);	
  free(write_buffer);
...
*******************************************************************************
*/
bmsStatus_t adBmsWriteData2950(uint8_t tIC, cell_asic *ic, uint8_t cmd_arg[2], TYPE_ADBMS2950 type, GRP group)
{
  bmsStatus_t status =  BMS_ERROR;
...
*******************************************************************************
*/
bmsStatus_t adBmsReadData2950(uint8_t tIC, cell_asic *ic, uint8_t cmd_arg[2], TYPE_ADBMS2950 type, GRP group)
{
  bmsStatus_t status = BMS_ERROR;
  ...
  if(group == ALL_GRP)
  {
    ...
    else if(type == Rdasall_2950){rBuff_size = RDASALL_SIZE; regData_size = RDASALL_SIZE;}
    else
    {
...
*******************************************************************************
*/
bmsStatus_t adBms6830_Adcv(RD rd, CONT cont, DCP dcp, RSTF rstf, OW_C_S owcs)
{
  bmsStatus_t status = BMS_ERROR;
...
*******************************************************************************
*/
bmsStatus_t adBms6830_Adax(OW_AUX owaux, PUP pup, CH ch)
{
  bmsStatus_t status = BMS_ERROR;
