...
void adBms6830ParseCell(uint8_t tIC, cell_asic *ic, GRP grp, uint8_t *cv_data)
{
  ...
  for(uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++)
  {
    ...
    case E_ADBMS: /* Cell Register group E */
    ...
    case F: /* Cell Register group F */
      ...
      ic[curr_ic].cell.c_codes[16] = (data[2] + (data[3] << 8));
      ic[curr_ic].cell.c_codes[17] = (data[4] + (data[5] << 8));
      break;
...
void adBms6830ParseAux(uint8_t tIC, cell_asic *ic, GRP grp, uint8_t *aux_data)
{
  ...
  for(uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++)
  {
    ...
    case B: /* Aux Register group B */
      ic[curr_ic].aux.a_codes[10] = (data[0] + (data[1] << 8));
      ic[curr_ic].aux.a_codes[11] = (data[2] + (data[3] << 8));
    ...
    case D: /* Aux Register group D */
      ...
      // ic[curr_ic].aux.a_codes[4] =  (data[2] + (data[3] << 8));      // VMV
      // ic[curr_ic].aux.a_codes[11] =  (data[4] + (data[5] << 8));     // VPV
      break;

    case E_ADBMS: /* Aux Register group E */
...
void adBms6830ParseStatusA(uint8_t tIC, cell_asic *ic, uint8_t *data)
{
  ...
  for(uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++)
  {
    ...
    // ic[curr_ic].stata.vref3   = (ic[curr_ic].stat.rx_data[4] | (ic[curr_ic].stat.rx_data[5] << 8));    // reserved
  }
}
...
void adBms6830ParseStatus(uint8_t tIC, cell_asic *ic, GRP grp, uint8_t *data)
{
  ...
  switch (grp)
  {
    ...
    case E_ADBMS: /* Status Register group E */
...
{
  // ic[1].tx_cfga.dcc =  0xFFFF;

  for(uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++)
  {
    // so ic[curr_ic].configa.tx_data[0] will be loaded with ((refon & 0x01) << 7) | (cth & 0x07) 
                                                              /*selecting the LSB bit and left shifting it to the exterme left side. taking it to the MSB side*/ /*this sets the LSB 3 bits*/
    ...
    ic[curr_ic].configa.tx_data[2] = (((ic[curr_ic].tx_cfga.soakon & 0x01) << 7) | ((ic[curr_ic].tx_cfga.owrng & 0x01) << 6) | ((ic[curr_ic].tx_cfga.owa & 0x07) << 3) | (ic[curr_ic].tx_cfga.dcc & 0x03)); 
...
void adBms6830CreateConfigb(uint8_t tIC, cell_asic *ic)
{
  for(uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++)
  {
    ...
    ic[curr_ic].configb.tx_data[4] = ((ic[curr_ic].tx_cfgb.dcc & 0xFF));    // DCC discharge is handled in 2 8bit registers | cell number 1 to 8 in register CFGBR4
    ic[curr_ic].configb.tx_data[5] = ((ic[curr_ic].tx_cfgb.dcc >> 8));      // and cell number 9 to 16 in CFGBR5
  }
}
