...
//   uint32_T CV_K;
  ...
  float RequestedCurrentScaling;
  float RequestedCurrentScalingFastCharging;
  float RequestedCurrentScalingSlow;
  uint8_T IR_WaitSOC;
  uint8_T IR_StartSOC;
  uint8_T IR_CycleCountThreshold;
  uint8_T IR_TransitionTime;
  uint8_T IR_PulseDropTime;
  }ChargeAlgorithm_t;
...
typedef struct thresholdData
{
    ...
    uint16_T PseudoLatchCurrent_mA;
...
typedef struct hardFaultMsg
{
   uint64_t hardFaultDataBuffer[16];
   uint8_t hardFaultmessage[128]; //payload
   uint16_t messageLen;
   uint32_t hardFaultWaitTime;
   bool sendHardFaultData;
}hardFaultMsg_t;
