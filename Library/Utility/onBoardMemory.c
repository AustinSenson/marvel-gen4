...
uint64_t ccWriteBufferFLASH[CC_BUFFER_SIZE]  = {0};
uint64_t ccReadBufferFLASH[CC_BUFFER_SIZE]   = {0};
uint64_t merlynFlashWriteBuff[4] = {0};

uint8_t recordedCycleCountMBD = 0;     //recorded cycle count for IR
double internalResistance = 0;         //Measured IR 
...
uint64_t hardFaultReadBufferFLASH[HARD_FAULT_SIZE] = {0};
uint64_t hardFaultWriteBufferFlash[HARD_FAULT_SIZE] = {0};
    ...
    switch(memorySection)
    {
        case SOC_SECTION:
            ...
            for(int i = 0; i < sizeof(int32_T); i++) //i < 4, number of bytes - all data types are of 4 bytes
            {
                ...
                memcpy(&writeBufferEEPROM[20], (uint8_t*)&ccOutput->CycleCount, sizeof(float));  //CycleCount on EEPROM
            
                writeBufferEEPROM[24]           =  recordedCycleCountMBD;      //recorded cycle for IR
                memcpy(&writeBufferEEPROM[25], (uint8_t*)&internalResistance, sizeof(double));
    ...
    switch(memorySection)
    {
        case SOC_SECTION:
            ...
            for(int i = 0; i < 4; i++)
            {
                ...
                memcpy(&ccOutput->CycleCount, &readBufferEEPROM[20], sizeof(float));  //CycleCount on EEPROM
                recordedCycleCountMBD                = readBufferEEPROM[24];
                memcpy(&internalResistance, &readBufferEEPROM[25], sizeof(double));  //CycleCount on EEPROM
