...
bmsStatus_t initializeFlashConfigData(__IO uint64_t *data)
{
	...
	data[PRECHARGE_RETRY_TIMEOUT]				= PRECHARGE_TIMEOUT_RETRY_mSec;
	data[PRECHARGE_RETRY_LIMIT]					= PRECHARGE_RETRY_LIMIT_COUNT;
	data[PRECHARGE_TIMEOUT]						= PRECHARGE_TIMEOUT_VALUE;
...
bmsStatus_t writeDataToFlash(uint8_t sectionType, uint32_t flashAddress, __IO uint64_t *flashData, uint16_t dataSize, uint8_t *receivedData)
{
	...
	switch(sectionType)
	{
		case FLASH_CONFIG_SECTION:	// this section be be only for debug purposes. Manual config change | knowledge of index is required
...
bmsStatus_t checkFlashData(uint8_t sectionType, uint32_t flashAddress, __IO uint64_t *flashData, uint16_t dataSize)
{
	...
	status = checkFirmwareConfigCompatibility(flashData);

	// should initialize default config in the case of crc mismatch and incompactible config version | firmware-config compactibility matrix has to be maintained
 ...
 * @retval BMS status
 */
bmsStatus_t checkFirmwareConfigCompatibility(__IO uint64_t *flashData)
{
	...
	*/
	...
	if(configMajorVersion >= MINIMUM_REQUIRED_CONFIG)			// this has to be changed according to the firmware version compactibility
	{
		status = BMS_OK;
	}
