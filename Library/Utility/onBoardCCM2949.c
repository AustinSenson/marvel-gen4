...
bmsStatus_t spi_read_calc_pec(uint16_t * pec15, uint8_t *datarx)
{
  bmsStatus_t status = BMS_ERROR;
...
bmsStatus_t CheckDeviceReady(void)
{
    ...
    )
  {
    ...
    delayMilliSeconds(1);//delayMilliSeconds(LTC2949_TIMING_CONT_CYCLE);
    error |= Cont(true);
...
bmsStatus_t LTC2949_reset(void)
{
  ...
  //if(status != BMS_OK) return status;
  delayMilliSeconds(1);//delayMilliSeconds(LTC2949_TIMING_CONT_CYCLE); // delay 15 ms
  status = LTC2949_WRITE(LTC2949_REG_OPCTRL, 1, (uint8_t *)LTC2949_BM_OPCTRL_SLEEP); // go to sleep
  //if(status != BMS_OK) return status;
  delayMilliSeconds(1);//delayMilliSeconds(LTC2949_TIMING_IDLE_CYCLE); // wait 19 ms for sleep
