...
*******************************************************************************
*/
bmsStatus_t adBms2950_Adi1(RD rd, CONT cont, OW ow)
{
...
*
* @details This function Set configuration gpo_2950 as pull high/Low.
*
* Parameters:
*
* @param [in]  gpo_2950      Enum type GPO Pin.
*
* @param [in]  set_clr     Enum type gpo_2950 set (Low or High).
*
* @return Gpo_value
*
*******************************************************************************
...
void adBms2950ParseConfiga(uint8_t tIC, cell_asic *ic, uint8_t *data)
{
  ...
  for(uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++)
  {
    ...
    ic[curr_ic].rx_cfga.gpo_2950    = ((ic[curr_ic].configa.rx_data[3] & 0xE0) >> 4) | ((ic[curr_ic].configa.rx_data[4] & 0x03) << 4) | (ic[curr_ic].configa.rx_data[3] & 0x01) ;
...
void adBms2950VrxParseData(uint8_t tIC, cell_asic *ic, GRP grp, uint8_t *vrx_data)
{
  ...
  if(data == NULL)
  {
    // printMsg("Failed to allocate parse vrx memory");
    exit(0);
  }
  for(uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++)
  {
...
void adBms2950ParseStatusE(uint8_t tIC, cell_asic *ic, uint8_t *data)
{
  ...
  for(uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++)
  {
    ...
    ic[curr_ic].state.gpo_2950   = ((ic[curr_ic].ste.rx_data[5] & 0x03) << 4 | ((ic[curr_ic].ste.rx_data[4] & 0xE0) >> 4) | (ic[curr_ic].ste.rx_data[4] & 0x01));
...
void adBms2950CreateConfiga(uint8_t tIC, cell_asic *ic)
{
  for(uint8_t curr_ic = 0; curr_ic < tIC; curr_ic++)
  {
    ...
    ic[curr_ic].configa.tx_data[3] = (((ic[curr_ic].tx_cfga.gpo_2950 & 0x0E) << 4) | ((ic[curr_ic].tx_cfga.gpio & 0x0F) << 1) | (ic[curr_ic].tx_cfga.gpo_2950 & 0x01));   /* GPO1 is at position 0 */
    ic[curr_ic].configa.tx_data[4] = ((ic[curr_ic].tx_cfga.gpo_2950 & 0x30) >> 4);
